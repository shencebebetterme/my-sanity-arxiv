...
function addPapers(num, dynamic) {
  ...
  for(var i=0;i<num;i++) {
    ...
    */
    ...
    //////////////////////
    // modified 2019-10-02, use regular expression to extract arxiv no.
    // p.link = http://arxiv.org/abs/1910.00520v1
    // p.link = http://arxiv.org/abs/hep-th/9608063v1
    // p.link = http://arxiv.org/abs/math/9906167v2

    //var rm=p.link.substring(21,);
    //if (rm.indexOf(".")==-1) {rm=rm.substring(7,)};

    var arxiv_pattern = /\d\S+\d/i;
    var rm = p.link.match(arxiv_pattern);

    var pdf_path = "../data/pdf/".concat(rm).concat(".pdf");
    ...
    title_link.on('click',function(elt){
      return function(){
        ...
        //pass argument titleRe to /titlepage at the backend serve.py
    ...
    //for example p.pid = 1906.04853.v1
