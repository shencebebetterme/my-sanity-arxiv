...
if not os.path.exists(Config.tmp_dir4): os.makedirs(Config.tmp_dir4)
...
for i,p in enumerate(pdf_files):
  ...
  if os.path.isfile(os.path.join(Config.tmp_dir4, 'thumb-0.png')):
    for i in range(8):
      f = os.path.join(Config.tmp_dir4, 'thumb-%d.png' % (i,))
      f2= os.path.join(Config.tmp_dir4, 'thumbbuf-%d.png' % (i,))
      if os.path.isfile(f):
        cmd = 'mv %s %s' % (f, f2)
        os.system(cmd)
  ...
  pp = Popen(['convert', '%s[0-7]' % (pdf_path, ), '-thumbnail', 'x156', os.path.join(Config.tmp_dir4, 'thumb.png')])
  t0 = time.time()
  ...
  if not os.path.isfile(os.path.join(Config.tmp_dir4, 'thumb-0.png')):
    ...
    cmd = "montage -mode concatenate -quality 80 -tile x1 %s %s" % (os.path.join(Config.tmp_dir4, 'thumb-*.png'), thumb_path)
    print(cmd)
    os.system(cmd)
